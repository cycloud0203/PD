...
static unsigned rngSeedFromEnvOrTime()
{
    const char* p = std::getenv("FP_SEED");
    if (p && p[0] != '\0') {
        char* end = nullptr;
        unsigned long v = std::strtoul(p, &end, 10);
        if (end != p && *end == '\0' && v <= 0xFFFFFFFFul) return (unsigned)v;
    }
...
void Floorplanner::floorplan()
{
    ...
    const std::vector<double> exploreAlphas = {0.1, 0.3, 0.5, 0.7, 0.9};
    std::vector<double> threadAlphas;
    if (numThreads > (int)exploreAlphas.size()) {
        threadAlphas.assign(numThreads, _alpha);
        for (int j = 0; j < (int)exploreAlphas.size(); ++j) {
            int idx = topK + j;
            if (idx < numThreads) threadAlphas[idx] = exploreAlphas[j];
        }
    } else {
        for (int t = 0; t < numThreads; ++t) threadAlphas.push_back(exploreAlphas[t]);
    }

    std::vector<std::thread> workers;
    for (int t = 0; t < numThreads; ++t) {
        double threadAlpha = threadAlphas[t];
        if (t < (int)topCandidates.size()) {
            auto nodes = topCandidates[t].nodes;
            int root = topCandidates[t].root;
            workers.emplace_back([this, deadline, baseSeed, t, threadAlpha,
