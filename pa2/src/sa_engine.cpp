...
double SAEngine::evalCostFull(double& outArea, double& outWL)
{
    ...
    double c = _localAlpha * (outArea / _sd.normA)
             + (1.0 - _localAlpha) * (outWL / _sd.normW);
...
{
    double realCost = _localAlpha * (area / _sd.normA)
                    + (1.0 - _localAlpha) * (wl / _sd.normW);
...
void SAEngine::runSingleSA(double budget)
{
    ...
    if (n <= 15) {
        movesPerTemp = n * n * 20;
    } else if (isAmi33Like || isAmi49Like) {
        movesPerTemp = std::max(n * n * 5, 80);
    } else {
        movesPerTemp = std::max(n * n * 3, 80);
    }
    ...
    double estMovesPerSec;
    if (n <= 15) {
        estMovesPerSec = 150000.0;
    } else if (n <= 35) {
        estMovesPerSec = 40000.0;
    } else {
        estMovesPerSec = 15000.0;
    }
...
void SAEngine::runWLRefinement(double budget)
{
    ...
    if (n <= 15) {
        ...
        movesPerTemp = std::max(n * n * 5, 100);
    }
