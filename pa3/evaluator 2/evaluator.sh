...
# File       [ evaluator.sh ]
# Author     [ FrankChiang ]
# Synopsis   [ To evaluate the score with pd25 ]
...
# check files
BASEDIR=$(dirname "$0")
FILE_LIST=("$BASEDIR/pd25_results.list")
for FILE_NAME in ${FILE_LIST[@]}; do
    if [ ! -f $FILE_NAME ]; then
        echo "[Eval] error: Please check file '$FILE_NAME' exist."
        echo "(ps. pd25_result.list should be in the same dir with evaluator.sh)"
        exit 1
    fi
done
